...
// Las modificaciones pedidas están en la siguiente función

const actualizarDatos = (datos) => {
  const nuevosDatos = datos;
  const arrayIntereses = calcularIntereses(datos);
  nuevosDatos.map((item, index) => {
    item["intereses"] = arrayIntereses[index];
  });

  //Se pueden ver en la consola luego de clickear el botón "calcular intereses"
